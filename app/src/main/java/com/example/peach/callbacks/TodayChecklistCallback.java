...
public class TodayChecklistCallback implements Callback<List<CurrentTaskModel>>
{
    private Context context;
    private ListView listView;

    public TodayChecklistCallback(Context context, ListView listView)
    {
    ...
    public void onResponse(Call<List<CurrentTaskModel>> call, Response<List<CurrentTaskModel>> response)
    {
        if (response.body() != null || ServerError.success(response.code()))
        {
            ...
            ArrayAdapter<CurrentTaskModel> adapter = new ArrayAdapter<>(this.context, android.R.layout.simple_list_item_1, list);
